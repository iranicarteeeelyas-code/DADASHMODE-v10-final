...
ILL.distance=(t,cx,cy,s,step)=>{/* V7 fix: bucket at the far end, the FARTHER the throwing line, the MORE seconds (2m +5 · 3m +10 · 4m +20) */
  const cols=[DIST.green,DIST.yellow,DIST.red];const bx=cx-520*s,by=cy+40*s;const lx=m=>bx+m*230*s;
  g.save();g.fillStyle=hexA(TH().ink,.07);g.fillRect(bx-120*s,cy-190*s,1240*s,380*s);g.restore();
  g.save();g.strokeStyle='#ff7a1a';g.lineWidth=10*s;g.beginPath();g.ellipse(bx,by-60*s,70*s,18*s,0,0,7);g.stroke();g.strokeStyle=hexA('#ffffff',.6);g.lineWidth=3*s;for(let i=-3;i<=3;i++){g.beginPath();g.moveTo(bx+i*20*s,by-54*s);g.lineTo(bx+i*12*s,by+20*s);g.stroke()}g.restore();
  cols.forEach((d,i)=>{const k=eo(ph(t,.2+i*.25,.6));const x=lx(2+i);g.save();g.globalAlpha=k;g.fillStyle=d.col;g.fillRect(x-9*s,cy-150*s,18*s,300*s);g.font=F(64*s);g.textAlign='center';g.textBaseline='middle';g.lineWidth=10*s;g.strokeStyle='#111';g.strokeText('+'+fa(d.sec),x,cy-200*s);g.fillStyle=d.col;g.fillText('+'+fa(d.sec),x,cy-200*s);g.font=F(34*s);g.fillStyle=TH().ink;g.fillText(fa(2+i)+' متر',x,cy+190*s);g.restore()});
  g.save();g.strokeStyle=TH().accent2;g.fillStyle=TH().accent2;g.lineWidth=6*s;const ay=cy+250*s;g.beginPath();g.moveTo(lx(2),ay);g.lineTo(lx(4)+90*s,ay);g.stroke();g.beginPath();g.moveTo(lx(4)+110*s,ay);g.lineTo(lx(4)+80*s,ay-14*s);g.lineTo(lx(4)+80*s,ay+14*s);g.closePath();g.fill();g.font=F(38*s);g.textAlign='center';g.fillText('دورتر = امتیاز بیشتر',lx(3),ay+50*s);g.restore();
  const idx=Math.floor((t%3.6)/1.2),bt=((t%3.6)%1.2)/.9;if(bt<=1){const sx=lx(2+idx),sy=by;const x=sx+(bx-sx)*bt,y=sy+(by-60*s-sy)*bt-Math.sin(bt*Math.PI)*(200+idx*60)*s;g.save();g.fillStyle='#ff8a3d';g.beginPath();g.arc(x,y,26*s,0,7);g.fill();g.strokeStyle='#5a2400';g.lineWidth=3*s;g.beginPath();g.moveTo(x-26*s,y);g.lineTo(x+26*s,y);g.stroke();g.restore()}
  if(step===1){vIcon('lock',cx+470*s,cy-200*s,150*s,{col:TH().accent2})}
  if(step===2){for(let i=0;i<3;i++){g.save();g.beginPath();g.arc(cx-120*s+i*120*s,cy+330*s,34*s,0,7);g.fillStyle=TH().accent;g.globalAlpha=eo(ph(t,i*.3,.4));g.fill();g.restore()}}};
...
ILL.sandwich=(t,cx,cy,s,step)=>{const th=TH();[-1,1].forEach((d,i)=>{const k=eo(ph(t,.1+i*.15,.6));if(k<=0)return;const x=cx+d*230*s,y=cy+40*s;const bite=step>=2?Math.min(.6,((t*.5)%1.2)):0;const w=1-bite;g.save();g.globalAlpha*=k;
  g.beginPath();g.ellipse(x,y+62*s,175*s,46*s,0,0,7);g.fillStyle='#f4efe6';g.fill();g.lineWidth=4*s;g.strokeStyle=hexA(th.ink,.25);g.stroke();
  g.fillStyle='#d9a55a';g.beginPath();g.ellipse(x,y+22*s,140*s*w,30*s,0,0,7);g.fill();g.fillStyle='#fff3c4';g.fillRect(x-132*s*w,y-4*s,264*s*w,16*s);g.fillStyle='#ff9fb0';g.fillRect(x-128*s*w,y-14*s,256*s*w,10*s);
  g.fillStyle='#e8b86a';g.beginPath();g.ellipse(x,y-18*s,140*s*w,34*s,0,Math.PI,0);g.fill();g.restore()});
  const tk=eo(ph(t,.5,.6));if(tk>0){g.save();g.globalAlpha*=tk;g.font=F(Math.round(96*s));g.textAlign='center';g.textBaseline='middle';g.fillStyle=th.accent2;g.fillText(fa(Math.max(0,60-Math.floor(t*6)%61)),cx,cy-200*s);g.restore()}};
ILL.glue=(t,cx,cy,s,step)=>{const th=TH();
...
ILL.shop=ILL.powers;ILL.risk=ILL.distance;ILL.case=ILL.vault;ILL.mitts=ILL.glue;
